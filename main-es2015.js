...
class AppComponent {
        ...
        this.GetThumbnail = (e) => {
            ...
            img.onload = () => {
                ...
                if (myCan.getContext) {
                    ...
                    this.thumbnail = myCan.toDataURL();
                    var blobFile = this.dataURItoBlob(this.thumbnail);
                    blobFile.name = 'thumbnail.jpg';
                    if (this.thumbnail != null && this.thumbnail != undefined) {
                        this.apiService.uploadFiles(this.Awsfile, blobFile, this.Customers_.Customers_Id, this.Album_.Album_Id).then((s) => {
                            ;
                            this.FileIndex++;
                            if (this.FileGroup.length > this.FileIndex) {
                                this.uploadFile();
                            }
                            setTimeout(() => {
                                this.thumbnail = s.thub.fileurl;
                                //It will tak some time
                                this.fileurl = s.fils.fileurl;
                                this.message = "saved";
                                this.file = "";
                            }, 1000);
                        });
    ...
    fileChange($event) {
        debugger;
        const file = $event.target.files; // (event.target as HTMLInputElement).files;
        // this.ImageFile = file;
        this.Awsfile = file[0];
        this.FileGroup = file;
    ...
    Search_Album() {
        this.apiService.Search_Album('').subscribe(Rows => {
            //debugger
            this.Album_Data = Rows[0];
        }, Rows => {
        });
    }
    Search_Customers() {
        this.apiService.Search_Customers('').subscribe(Rows => {
            //debugger
            this.Customers_Data = Rows[0];
        }, Rows => {
        });
    }
    uploadFileold() {
...
class APIservice {
    ...
    uploadFiles(file, thumbnail, Customers_Id, Album_Id) {
        return new Promise((res, rej) => {
            debugger;
            this.uploadFile(file).then(fils => {
                console.log(fils);
                debugger;
                this.uploadFile(thumbnail).then(thub => {
                    debugger;
                    console.log(fils[0], thub[0]);
                    res({ fils, thub });
                    this.Save_Images_data(Customers_Id, Album_Id, fils[1], thub[1]).then(rs => {
                        res({ fils, thub });
                    }, er => {
                        rej(er);
                    });
    ...
    uploadFile(file) {
        return new Promise((res, rej) => {
            ...
            debugger;
            // this.http.get("http://localhost:3000/s3-path",
            this.http.get("http://album2843adminapi.trackbox.co.in:3545/s3-path", { params: { fileType: fileType[1] } }).subscribe((response) => {
                debugger;
    ...
    Save_Images_data(Customers_Id, Album_Id, File_Path, Thumbnail) {
        return new Promise((res, rej) => {
            this.http.post(_environments_environment_js__WEBPACK_IMPORTED_MODULE_1__["environment"].BasePath + "Public_Data/Save_Images_data", { params: { File_Path: File_Path, Thumbnail: Thumbnail,
                    Customers_Id: Customers_Id, Album_Id: Album_Id } }).subscribe((response) => {
                ;
                const { url } = response;
                if (url) {
                }
    ...
    Search_Customers(Customers_Name) {
